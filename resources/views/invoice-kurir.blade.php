...
<head>
    <meta charset="UTF-8">
    <title>Nota Penjualan (Kurir) #{{ $transaksi->no_nota_pembelian ?? $transaksi->id_pembelian }}</title>
    <style>
        body {
            font-family: 'DejaVu Sans', sans-serif; /* DejaVu Sans lebih baik untuk karakter Unicode di PDF */
            font-size: 11px; /* Sedikit lebih kecil untuk kerapian */
            margin: 15px;
            color: #333;
        }
        .container {
        ...
        table.info-table .label {
            font-weight: bold;
            width: 120px; /* Lebar label konsisten */
        }
        ...
        .totals-section .label {
             /* text-align: right; */ /* Label di kiri saja */
        }
        ...
        /* Clearfix for floats if needed, though tables might avoid this */
        .clearfix::after {
            content: "";
            clear: both;
            display: table;
    ...
    <div class="container">
        ...
        <table class="info-table section">
            ...
            <tr>
                ...
                <td>{{ $transaksi->tanggal_pengiriman ? \Carbon\Carbon::parse($transaksi->tanggal_pengiriman)->format('d M Y') : '-' }}</td>
            </tr>
        </table>

        <div class="section">
            <div class="section-title">Informasi Pembeli & Pengiriman</div>
            <table class="info-table">
                ...
                <tr>
                    ...
                    <td>{{ $transaksi->alamat ?? 'Alamat tidak tersedia' }}</td>
                </tr>
                <tr>
                    <td class="label">Kurir:</td>
                    <td>Kurir ReUseMart ({{ $transaksi->kurir->nama ?? 'Belum Ditugaskan' }})</td>
                </tr>
            </table>
        </div>
                ...
                <tbody>
                    @if($transaksi->barangTitipan->isNotEmpty())
                        ...
                        <tr>
                            ...
                            <td class="text-center">1</td> {{-- Asumsi kuantitas selalu 1 per record barangTitipan --}}
        ...
        <div class="totals-section clearfix">
            <table>
                ...
                <tr>
                    ...
                    <td class="value">Rp {{ number_format($transaksi->ongkir ?? 0, 0, ',', '.') }}</td>
                </tr>
                @if(isset($transaksi->poin_diskon) && $transaksi->poin_diskon > 0)
                <tr>
                    <td class="label">Potongan Poin ({{ $transaksi->poin_diskon }} poin):</td>
                    <td class="value">- Rp {{ number_format($transaksi->poin_diskon * 100, 0, ',', '.') }}</td> {{-- Asumsi 1 poin = Rp 100 --}}
                </tr>
                @endif
        ...
        <div class="signature-section">
            <table>
                <tr>
                    <td>Disiapkan Oleh,<br>(QC ReUseMart)</td>
                    <td>Kurir,</td>
                    <td>Diterima Oleh,<br>(Pembeli)</td>
                </tr>
                <tr>
                    <td><br><br><br>({{ Auth::guard('pegawai')->user()->nama ?? 'Staff Gudang' }})</td>
                    <td><br><br><br>({{ $transaksi->kurir->nama ?? '_________________' }})</td>
