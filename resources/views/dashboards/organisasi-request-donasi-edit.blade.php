    ...
    <div class="flex h-screen bg-gray-100">
        ...
        <main class="flex-1 p-6 overflow-auto">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-semibold text-gray-800">Edit Donation Request: {{ $requestDonasi->id_request }}</h1>
            ...
            <div class="bg-white p-6 rounded-lg shadow max-w-4xl mx-auto">
                ...
                <form method="POST" action="{{ route('organisasi.request-donasi.update', $requestDonasi->id_request) }}" class="grid grid-cols-1 gap-6">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="deskripsi" class="block text-sm font-medium text-gray-700">Description</label>
                        <textarea name="deskripsi" id="deskripsi" class="mt-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full" rows="4" required>{{ old('deskripsi', $requestDonasi->deskripsi) }}</textarea>
                    </div>
                    <div class="flex justify-end space-x-4">
                        <a href="{{ route('organisasi.request-donasi.index') }}" class="text-gray-600 hover:text-gray-800 font-medium">Cancel</a>
                        <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition duration-200">Update Donation Request</button>
                    </div>
                </form>
            </div>
        </main>
