    ...
    <div class="flex h-screen bg-gray-100">
        ...
        <aside class="w-64 bg-white shadow-lg">
            <div class="p-4 border-b">
                <h2 classdotcom">ReUseMart - CS</h2>
            </div>
            <nav class="mt-6">
        ...
        <main class="flex-1 p-6 overflow-auto">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-semibold text-gray-800">Edit Penitip: {{ $penitip->id_penitip }}</h1>
            ...
            <div class="bg-white p-6 rounded-lg shadow max-w-4xl mx-auto">
                ...
                <form method="POST" action="{{ route('cs.penitip.update', $penitip->id_penitip) }}" class="grid grid-cols-1 md:grid-cols-2 gap-6" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="no_ktp" class="block text-sm font-medium text-gray-700">KTP Number</label>
                        <input type="text" name="no_ktp" id="no_ktp" class="mt-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full" value="{{ old('no_ktp', $penitip->no_ktp) }}" required>
                    </div>
                    <div>
                        <label for="nama" class="block text-sm font-medium text-gray-700">Name</label>
                        <input type="text" name="nama" id="nama" class="mt-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full" value="{{ old('nama', $penitip->nama) }}" required>
                    </div>
                    <div>
                        <label for="telepon" class="block text-sm font-medium text-gray-700">Phone</label>
                        <input type="text" name="telepon" id="telepon" class="mt-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full" value="{{ old('telepon', $penitip->telepon) }}" required>
                    </div>
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                        <input type="email" name="email" id="email" class="mt-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full" value="{{ old('email', $penitip->email) }}" required>
                    </div>
                    <div>
                        <label for="password" class="block text-sm font-medium text-gray-700">Password (Leave blank to keep unchanged)</label>
                        <input type="password" name="password" id="password" class="mt-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full">
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2 flex justify-end space-x-4">
                        ...
                        <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition duration-200">Update Penitip</button>
                    </div>
                </form>
            </div>
        </main>
